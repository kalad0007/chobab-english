...
import { CATEGORY_LABELS, TOEFL_SECTION_MAX, toeflLevel, xpToLevel, levelTitle } from '@/lib/utils'
...
const SECTION_COLORS: Record<string, { bg: string; text: string; bar: string }> = {
  reading: { bg: 'bg-blue-500', text: 'text-blue-600', bar: 'bg-blue-500' },
  listening: { bg: 'bg-emerald-500', text: 'text-emerald-600', bar: 'bg-emerald-500' },
  speaking: { bg: 'bg-orange-500', text: 'text-orange-600', bar: 'bg-orange-500' },
  writing: { bg: 'bg-purple-500', text: 'text-purple-600', bar: 'bg-purple-500' },
}
...
export default async function StudentDashboard() {
  ...
  const { data: recentSubmissions } = await supabase
    .from('submissions')
    .select('id, score, total_points, percentage, submitted_at, status, exams(title)')
    .eq('student_id', user.id)
  ...
  Object.keys(CATEGORY_LABELS).forEach(cat => {
    ...
    sectionScores[cat] = stat ? Math.round((stat.accuracy / 100) * TOEFL_SECTION_MAX[cat]) : 0
  })
  const estimatedTotal = Object.values(sectionScores).reduce((a, b) => a + b, 0)
  const hasAnyStats = (skillStats ?? []).length > 0
  ...
  return (
    <div className="p-4 md:p-7">
      <div className="mb-6 pt-2 md:pt-0">
        ...
        <p className="text-gray-500 text-sm mt-1">TOEFL iBT 목표를 향해 화이팅!</p>
      </div>
      ...
      <div className="bg-gradient-to-r from-slate-800 to-slate-700 rounded-2xl p-5 mb-6">
        <div className="flex items-center gap-5 mb-4">
          <div className="w-16 h-16 rounded-full bg-gradient-to-br from-amber-400 to-orange-500 flex items-center justify-center flex-shrink-0 shadow-lg">
            <span className="font-black text-2xl text-white">{estimatedTotal}</span>
          </div>
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              <h3 className="text-white font-bold text-lg">예상 TOEFL 점수</h3>
              <span className="text-xs bg-white/20 text-white px-2 py-0.5 rounded-full">/ 120</span>
            </div>
            <p className="text-slate-400 text-sm">{hasAnyStats ? toeflLevel(estimatedTotal) : '아직 데이터가 없어요'}</p>
          </div>
        ...
        <div className="grid grid-cols-4 gap-3">
          {Object.keys(CATEGORY_LABELS).map(cat => {
            const score = sectionScores[cat]
            const max = TOEFL_SECTION_MAX[cat]
            ...
            return (
              <div key={cat} className="text-center">
                ...
                <div className="bg-white/10 rounded-full h-1.5 mt-1">
                  <div className={`${colors.bar} h-1.5 rounded-full transition-all`} style={{ width: `${pct}%` }} />
                </div>
        ...
        <div className="mt-4 pt-3 border-t border-white/10">
          <div className="flex items-center gap-2">
            <span className="text-xs text-slate-400">Lv.{level} {levelTitle(level)}</span>
            <div className="flex-1 bg-white/10 rounded-full h-1.5">
              <div className="bg-gradient-to-r from-amber-400 to-yellow-300 h-1.5 rounded-full transition-all" style={{ width: `${xpProgress}%` }} />
            </div>
        ...
        {Object.keys(CATEGORY_LABELS).map(cat => {
          ...
          return (
            ...
            >
              ...
              <p className="text-white/70 text-xs">{sectionScores[cat]}/{TOEFL_SECTION_MAX[cat]}점</p>
            </Link>
          )
        })}
      </div>
              ...
              Object.keys(CATEGORY_LABELS).map(cat => {
                ...
                return (
                  <div key={cat} className="flex items-center gap-3">
                    ...
                    <span className="w-16 text-sm font-semibold text-gray-700 flex-shrink-0">
                      {CATEGORY_LABELS[cat]}
                    </span>
                    <div className="flex-1 bg-gray-100 rounded-full h-2.5">
                      <div
                        className={`h-2.5 rounded-full transition-all ${!hasData ? 'bg-gray-200' : colors.bar}`}
                        style={{ width: `${accuracy}%` }}
                      />
                    </div>
                    <span className={`w-9 text-right text-sm font-bold flex-shrink-0 ${!hasData ? 'text-gray-300' : colors.text}`}>
                ...
                (pendingExams ?? []).map(exam => (
                  <Link key={exam.id} href={`/student/exams/${exam.id}`}
                    ...
                    <div>
                      ...
                      {exam.time_limit && <p className="text-xs text-gray-400 mt-0.5">{exam.time_limit}분</p>}
                    </div>
