...
import { updatePassage, type Annotation } from '../../actions'
...
interface ParagraphState {
  id: string; text: string; text_ko: string
  annotations: Annotation[]; mode: 'edit' | 'annotate'; translating: boolean
}
interface Toolbar {
  x: number; y: number; paraId: string; start: number; end: number; step: 'main' | 'vocab'
}
interface VocabWord {
...
function getTextOffset(container: Node, targetNode: Node, targetOffset: number): number {
  ...
  let total = 0; let node = walker.nextNode()
  while (node) {
    if (node === targetNode) return total + targetOffset
    total += node.textContent?.length ?? 0; node = walker.nextNode()
  }
...
function AnnotatedView({ para, onMouseUp }: {
  para: ParagraphState
  ...
  return (
    <div ref={ref} onMouseUp={handleMouseUp}
      ...
      {parts.map((p, i) => {
        ...
        if (p.type === 'chunk') return <span key={i}><span className="bg-sky-100 text-sky-800 rounded px-0.5">{p.text}</span><span className="text-gray-400 font-bold mx-1 select-none">/</span></span>
        if (p.type === 'vocab') return <span key={i} className="text-purple-700 underline underline-offset-2 decoration-dotted cursor-pointer">{p.text}</span>
