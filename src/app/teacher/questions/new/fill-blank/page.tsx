...
// ────────── 상수 ──────────
// 10단계 난이도 — DIFFICULTY_LEVELS (lib/utils)에서 가져옴
...
export default function FillBlankPage() {
  ...
  async function handleSave() {
    ...
    // 단락 텍스트: 빈칸은 te___ 형식으로 렌더링
    const contentText = tokens.map(t => t.isBlank ? blankDisplay(t) : t.text).join('')
    ...
    const { error: dbErr } = await supabase.from('questions').insert({
      ...
      content: contentText || 'Fill in the missing letters.',
      passage: JSON.stringify(tokens),
      options: null,
      answer: answers.join(','),
      explanation: JSON.stringify({ title, difficulty, timeLimit, format, explanation: explanationText }),
      source: 'teacher',
      audio_url: null,
      audio_script: null,
          ...
          <div key={si} className="flex items-baseline gap-3">
            ...
            <div className="text-base leading-8 select-none">
              {sent.map(t => {
                ...
                if (t.type === 'nonword') return display ? <span key={t.id} className="whitespace-pre">{display}</span> : null
                return (
                  <span key={t.id}
                    onClick={e => handleWordClick(t.id, e)}
                    className={`cursor-pointer rounded px-0.5 transition-all ${
        ...
        {groupSentences(tokens).map((sent, si) => (
          <div key={si} className="flex items-baseline gap-3">
            ...
            <div className="text-base leading-8 text-gray-900">
              {sent.map(t => {
                const display = t.text.replace(/\n/g, '')
                if (t.type === 'nonword') return display ? <span key={t.id}>{display}</span> : null
                return t.isBlank ? <BlankInput key={t.id} token={t} /> : <span key={t.id}>{t.text}</span>
              })}
            </div>
          </div>
        ))}
              ...
              <div className="flex flex-wrap gap-1">
                {DIFFICULTY_LEVELS.map(l => (
                  <button key={l.value} type="button" onClick={() => setDifficulty(l.value)}
                    title={`${l.name} (${l.cefr})`}
                    className={`py-1.5 px-2 rounded-lg text-xs font-bold transition flex flex-col items-center gap-0.5 min-w-[2.8rem] ${
                      l.value === difficulty
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-5">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="각 빈칸의 정답 근거나 문법 포인트를 설명해주세요..."
              rows={3}
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            ...
            <div className="rounded-xl border-2 border-dashed border-gray-200 bg-gray-50 p-5">
              ...
              <p className="text-sm font-bold text-gray-700 mb-4">
                {format === 'sentences'
                  ? 'Fill in the missing letters in each sentence.'
